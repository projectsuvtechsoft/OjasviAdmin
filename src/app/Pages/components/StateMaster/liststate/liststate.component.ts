...
export class ListstateComponent implements OnInit {
  ...
  add(): void {
    ...
    this.api.getAllStateMaster(1, 1, 'SEQUENCE_NO', 'desc', '').subscribe(
      (data) => {
        if (data['count'] == 0) {
  ...
  chekedproduct() {
    this.api
      .getAllStateMaster(1, this.totalRecords, this.sortKey, 'desc', '')
      .subscribe((res) => {
        if (res['code'] === 200) {
  ...
  bulkUpdate(fieldName: string, value: any) {
    ...
    this.api.stateBulkUpdate(payload).subscribe(
      (res: any) => {
        if (res.code == 200) {
  ...
  bulkDelete() {
    ...
    this.api.stateDeleteBulk(payload).subscribe(
      (res: any) => {
        if (res.code == 200) {
