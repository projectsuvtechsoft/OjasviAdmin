...
chekedproduct(){
   this.api.getAllUnitMaster(1, this.totalRecords, this.sortKey, 'desc', '')
      .subscribe(res => {
        if (res['code'] === 200) {
...
bulkUpdate(fieldName: string, value: any) {
  ...
  this.api.unitBulkUpdate(payload).subscribe( (res: any)  => {
           if (res.code == 200) {
...
const payload = {
  data: this.dataList
    ...
    .map(item => {
      const obj: any = {
        ID: item.ID,
        NAME: item.UNIT_NAME   // include NAME from dataList
      };
...
deleteSingleRecord(row: UnitMaster) {
    this.loadingRecords=true
